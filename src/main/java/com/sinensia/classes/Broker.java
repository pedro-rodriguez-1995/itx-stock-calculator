...
public void buyStocks(ArrayList<Stock> stocklist, BigDecimal investment) {
	...
	for(Stock stock : stocklist) {
		...
		System.out.println(stock.getDate()+" Stock bought at opening price of: "+stock.getOpprice().toString()+"€ for a total of: "+stocksbought.toString()
				+ " stocks Total stocks in this account: "+this.totalstocks.toString()+" Total commission: "+this.getTotalcommission().toString()+"€");
...
public void sellStocks(LocalDate date, ArrayList<Stock> stocklist) {
	
	for(Stock stock : stocklist) {
		
		if(stock.getDate().equals(date)) {
			
			BigDecimal benefit=this.getTotalstocks().multiply(stock.getClprice()).setScale(3,BigDecimal.ROUND_HALF_EVEN);
			
			System.out.println("You've sold all your: "+this.getTotalstocks()+" stocks at a price of: "+stock.getClprice()+"€ per stock"
					+ " for a total of: "+benefit+"€");
